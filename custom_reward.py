...
def biofilm_reward(self, x, u, con):
    Sp_i = 0
    cost = 0
    R = 4
    if not hasattr(self, 'u_prev'):
...
def four_tank_reward(self, x, u, con):
    ...
    for k in self.SP:
        ...
        # cost += (np.sum(x_normalized - setpoint_normalized[self.t]) ** 2) * r_scale.get(k, 1)
        cost += np.tanh(35 * (np.sum(x_normalized - setpoint_normalized[self.t]) ** 2)) * r_scale.get(k, 1)
...
def cstr_series_recycle_reward(self, x, u, con):
    Sp_i = 0
    cost = 0
    R = 0.001
    if not hasattr(self, 'u_prev'):
    ...
    for k in self.SP:
        ...
        # cost += (np.sum(x_normalized - setpoint_normalized[self.t]) ** 2) * r_scale.get(k, 1)
        cost += np.tanh(6 * (np.sum(x_normalized - setpoint_normalized[self.t]) ** 2)) * r_scale.get(k, 1)
