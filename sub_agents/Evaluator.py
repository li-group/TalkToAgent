...
class Evaluator:
    ...
    def evaluate(self, traj, traj_orig, query):
        """
        ...
        You will be given two trajectories in JSON format -- one original and one modified --, each represented as a list
        where each object corresponds to one timestep and includes all state and action values.
        Interpret these trajectories as sequential time-series.
        If you thought that the modified trajectory from the policy faithfully follow the intention from the user query, you can
        confirm the contrastive policy, otherwise raise an error by calling raise error tool.
