...
class make_env(gym.Env):
    ...
    def __init__(self, env_params):
        ...
        if self.normalise_o:
            self.observation_space = spaces.Box(low=np.array([-1] * base_obs_low.shape[0]),
        ...
        if env_params.get("disturbances") is not None:
            ...
            if self.normalise_o:
                self.observation_space = spaces.Box(low=np.array([-1] * extended_obs_low.shape[0]),
    ...
    def _scale_X(self, state):
        normstate = (2 * (state - self.observation_space_base.low)
                     / (self.observation_space_base.high - self.observation_space_base.low) - 1)
        return normstate

    def _scale_U(self, action):
        normaction = (2 * (action - self.env_params["a_space"]["low"])
                      / (self.env_params["a_space"]["high"] - self.env_params["a_space"]["low"]) - 1)
        return normaction
