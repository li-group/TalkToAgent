...
true_tools = ["FI"] * 20 + ["EO"] * 20 + ["CF_A"] * 20 + ["CF_B"] * 20 + ["CF_P"] * 20
...
for i, query in enumerate(total_queries):
    ...
    if choice.finish_reason == "function_call":
        ...
        if true_tool in ['FI', 'EO', 'CF_A']:
            ...
            if list(predicted_arg.values()) != list(true_arg.values()):
                print(
            ...
            if list(predicted_arg_.values()) != list(true_arg_.values()):
                print(
                    f"Misallocation in arguments in query: {query} \n GroundTruth: {true_arg} Prediction: {predicted_arg} \n")
    else:
...
cm = confusion_matrix(true_tools, predicted_tools, labels=labels)
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=labels)

disp.plot(cmap='Blues', values_format='d')
plt.title("Tool selection Confusion Matrix")
plt.tight_layout()
plt.savefig(f'./figures/[{MODEL}] Coordinator classification.png')
plt.show()
